'''
프린터는 인쇄하고자 하는 문서를 명령 받은 '순서대로'
즉, 먼저 요청된 것을 먼저 인쇄한다. 여러 개의 문서가 쌓인다면 Queue 자료구조에 쌓여서
FIFO - First In Frist Out 에 따라 인쇄가 된다.

근데 개발자 슬기가 새로운 프린터기 내부 소프트웨어를 개발했는데,
...
몇 번째에 놓여 있는지를 나타내는 정수 M(0 ≤ M < N)이 주어진다. 이때 맨 왼쪽은 0번째라고 하자. 두 번째 줄에는 N개 문서의 중요도가 차례대로 주어진다.
중요도는 1 이상 9 이하의 정수이고, 중요도가 같은 문서가 여러 개 있을 수도 있다.
'''

# 그렇다면 일단 내가 해야될건 파이썬의 큐 자료구조 사용방법 익히기
# 우선순위 큐를 사용해야한다. 우선순위 큐를 공부해야겠지?
